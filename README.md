...
-   `MAX_FILE_SIZE`: Maximum size (in Megabytes) for files to be processed. Files larger than this will be automatically skipped (default is `0` for no limit).
...
### Terminal Logging
During batch processing, the terminal prints real-time progress, file types, and file sizes in a single line so you can monitor operations:
```
[Processing] User: 123456789 | Msg ID: 1042 / 1100 | Type: Video | Size: 48.7 MB
